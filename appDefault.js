const STORE = {
    ...
    {
        // QUESTION 5
        question: `Roughly, how many times has the Beatles hit single 'Yesterday' officially been covered by other artists (it is still regarded as the most covered song of all-time)?`,
    ...
    },
    {
        ...
        answers: [`Sgt. Pepper`, 'Abbey Road', 'The White Album', 'Yellow Submarine'],
...
function generateQuestion() {
    ...
    <fieldset>
        <legend class='questions'>${questionInd.question}</legend><section class ='js-results'></section></div><button type='submit' tabindex='5' class ='submitButton'>Submit</button></fieldset></form>`);
...
function generateAnswers() {
    ...
    for (let i = 0; i < questionInd.answers.length; i++) {
        $('.js-results').append(`<p><input class ='choices' type='radio' tabindex = '${i}' id ='answers${i + 1}' value ='${questionInd.answers[i]}' name='answers' required>
...
function correctAnswer() {
    ...
    <button type ='button' tabindex='6' class ='next'>Next</button>`);
    $('.quizBox').hide();
}
